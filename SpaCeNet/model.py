...
class SpaCeNet:
  ...
  def _check_dims(X_mat: torch.Tensor, Theta_tens: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if not(X_mat.shape[1] == Theta_tens.shape[-1] == Theta_tens.shape[-2]):
      raise ValueError(f"Mismatch in observations of X_mat and Theta_tens: {X_mat.shape[1]} != {Theta_tens.shape[-2:]}")
  ...
  def fit(
    ...
    # allow warm start
    if reset_params or (self._params is None) or torch.any(torch.isnan(self._params)) or torch.all(self._params == 0):
  ...
  def _loss_grad(self, params: torch.Tensor) -> torch.Tensor:
    ...
    grad_mu = torch.tensordot(
      A_mat,
      Omega_mat + torch.tensordot(torch.sum(self.Theta_train, dim = -1), Drho_tens, dims = ([1], [0])),   # [S, a, j, q]
      dims = ([0, 1, 2], [0, 1, 2])
    )
    ...
    grad_Omega_diag = 0.5 * (
      self.n * self.S / Omega_diag - torch.sum(XM_mat ** 2, dim = (0, 1)) + torch.sum((A_mat - XM_mat) ** 2, dim = (0, 1))
    )
  ...
  def _prox(self, params_vec: torch.Tensor, step_size: float) -> torch.Tensor:
    ...
    params_mat[(self.p + self.n_triu):, :] = params_vec[(self.p + self.n_triu):].reshape((self.L, self.n_triu)).T   # Drho params
...
class SpaCeNetGridSearch:
  ...
  def __init__(
    ...
    n_beta: int = 4,
    n_refinements: int = 6,
    refinement_criterion: str = "score_test",
    penalize_diag: bool = False,
  ...
  def fit(
    ...
    for refinement in refinement_iterator:
      ...
      if (self.refinement_criterion == "score_test"):
        ...
        best_alpha, best_beta = result_df[["alpha", "beta"]].iloc[np.nanargmin(result_df[self.refinement_criterion])]
      else:
      ...
      if best_alpha_on_lower_bound:
          ...
          min_alpha = np.logspace(np.log10(all_alphas[best_alpha_ind - 1]), np.log10(all_alphas[best_alpha_ind]), 3)[1]
          max_alpha = np.logspace(np.log10(all_alphas[best_alpha_ind]), np.log10(all_alphas[best_alpha_ind + 1]), 3)[1]
